...
import { create, deleteMemo, getMemoAll, getMemoOne, update, } from "../controller/memo.js";
...
router.post(`/memo`, Token.verifyToken, async (req, res) => {
    try {
...
router.get(`/getMemoAll`, Token.verifyToken, async (req, res) => {
    try {
        console.log(req.user);
...
router.get(`/getMemoOne/:memoId`, Token.verifyToken, async (req, res) => {
    try {
        console.log(req.user);
        const memo = await getMemoOne(req);
        if (!memo)
            return res.status(401).json("指定のメモが存在しません");
        res.status(201).json(memo);
    }
    catch (error) {
        return res.status(500).json(error);
...
router.put(`/update/:memoId`, Token.verifyToken, async (req, res) => {
    try {
        console.log(req.user);
...
router.put(`/delete/:memoId`, Token.verifyToken, async (req, res) => {
    try {
        console.log(req.user);
